...
def run(file, outfile):
    ...
    with h5py.File(outfile, 'w') as out:
        ...
        for s in stream:
            ...
            to_store = {f"num_B_total_tracks" : None}
            for origin_key, label in base_labels.items():
                ...
                if origin_key in [3,4]:
                    if to_store[f"num_B_total_tracks"] is None:
                        to_store[f"num_B_total_tracks"] = num_origin.copy()
                    else:
                        to_store[f"num_B_total_tracks"] += num_origin
