    ...
    with h5py.File(outfile, 'w') as out:
        ...
        for s in stream:
            ...
            for origin_key, label in base_labels.items():
                ...
                if origin_key in [3,4,5]:
                    if to_store[f"num_HF_tracks"] is None:
                        to_store[f"num_HF_tracks"] = num_origin.copy()
                    else:
                        to_store[f"num_HF_tracks"] += num_origin
